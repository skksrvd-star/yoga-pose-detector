...
const PosePanel: React.FC<PosePanelProps> = ({
  ...
  return (
    <div className="lg:col-span-1">
      <motion.div
        layout
        className="bg-white rounded-lg sm:rounded-xl md:rounded-2xl shadow-xl p-3 sm:p-4 md:p-6 relative overflow-hidden"
      >
        {/* Subtle animated background pulse when match occurs */}
        <AnimatePresence>
          {isMatch && (
            <motion.div
              key="match-bg"
              className="absolute inset-0 bg-gradient-to-br from-green-100/40 to-green-200/30 rounded-2xl"
              initial={{ opacity: 0 }}
        ...
        <div className="relative z-10">
          ...
          <div className="bg-gradient-to-br from-purple-100 to-blue-100 rounded-lg md:rounded-xl p-3 sm:p-4 md:p-6 mb-3 sm:mb-4 md:mb-6">
            <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-2 sm:mb-3">
              ...
              >
                ...
                <motion.div
                  ref={imageDivRef}
                  className={`w-20 h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 flex-shrink-0 bg-white rounded-lg overflow-hidden shadow-md flex items-center justify-center border-2 ${
                    isMatch ? 'border-green-400' : 'border-transparent'
                  }`}
                  animate={{
                    boxShadow: isMatch
                      ? ['0 0 10px rgba(34,197,94,0.4)', '0 0 20px rgba(34,197,94,0.8)', '0 0 10px rgba(34,197,94,0.4)']
                      : '0 0 0 rgba(0,0,0,0)',
                  }}
              ...
              >
                ...
                <motion.div
                  ref={detectedImageDivRef}
                  className={`w-20 h-20 sm:w-28 sm:h-28 md:w-32 md:h-32 flex-shrink-0 bg-white rounded-lg overflow-hidden shadow-md flex items-center justify-center transition-all ${
                ...
                <div className="mt-2 text-center">
                  {isMatch ? (
                    <motion.div
                      ...
                      exit={{ opacity: 0 }}
                      transition={{ duration: 0.4 }}
                      className="inline-flex items-center gap-1 text-xs sm:text-sm font-semibold text-green-600"
                    >
            ...
            <div className="max-h-48 sm:max-h-64 md:max-h-96 overflow-y-auto">
              <ul className="grid grid-cols-3 gap-1.5 sm:gap-2">
                {posesData.map((p, idx) => (
                  ...
                  >
                    <div
                      className={`p-0.5 sm:p-1 rounded-md border-2 ${
                        selectedPoseIndex === idx
                          ? 'border-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.6)]'
                          : 'border-transparent'
          ...
          <div className="mt-3 sm:mt-4 md:mt-6 p-2 sm:p-3 md:p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-center">
